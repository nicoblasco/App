				...
				<div v-for="c in contactsFiltered" :key="c.id" class="flex contact" @click="openDialog(c)">
					...
					<div class="info box grow flex">
						<div class="name box grow flex column justify-center p-10">
							<div class="fullname fs-18"><strong>{{c.razonSocial}}</strong></div>
...
<script>
import UserDialog from '@/components/ProviderDialog'
...
export default {
	...
	computed: {
		contactsFiltered() {
			return this.datos.filter(({razonSocial, email, phone}) => (razonSocial+email+phone).toString().toLowerCase().indexOf(this.search.toString().toLowerCase()) !== -1)
		},
		contactsClass() {
          ...
          if (valid) {
            ...
            if (me.editedIndex === 0) {
                ...
                axios.put(this.URL_UPDATE,{
                    'Id': me.data.id,
					'RazonSocial':me.data.razonSocial,
